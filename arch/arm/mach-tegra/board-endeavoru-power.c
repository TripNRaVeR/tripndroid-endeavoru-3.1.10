...
TPS_PDATA_INIT(ldo3, common, 1000, 3300, 0, 0, 0, 0, -1, 0, 0, 0, PWR_OFF_ON_SLEEP, 0);
...
FIXED_REG(3, aud_a1v8_en, NULL, TEGRA_GPIO_PD2, true, 1800, 0, 0);
...
FIXED_REG(14, vdd_sdmmc3_2v85_en, NULL, TEGRA_GPIO_PM3, true, 2850, 1, 0);
...
static int __init enterprise_fixed_regulator_init(void)
{
	...
	if (board_info.fab < BOARD_FAB_A03) {
		pr_info("[TRIPNDROID] PWR: FIXED-REG: A02\n");
	} 
	else {
		pr_info("[TRIPNDROID] PWR: FIXED-REG: A03\n");
	}

	if (board_info.board_id == BOARD_E1197) {
		pr_info("[TRIPNDROID] PWR: BOARD-ID: BOARD E1197\n");
	}
	if (board_info.board_id == BOARD_E1205) {
		pr_info("[TRIPNDROID] PWR: BOARD-ID: BOARD E1205\n");
	}
	if (board_info.board_id == BOARD_E1239) {
		pr_info("[TRIPNDROID] PWR: BOARD-ID: BOARD E1239\n");
	}
	else {
		pr_info("[TRIPNDROID] PWR: BOARD-ID: UNKNOWN\n");
	}
#endif
...
#ifdef CONFIG_TRIPNDROID_DEBUG
	if ((board_info.fab < BOARD_FAB_A03) && (board_info.board_id != BOARD_E1239)) {
		pr_info("[TRIPNDROID] PWR: REGULATOR: A02\n");
	} 
	else {
		pr_info("[TRIPNDROID] PWR: REGULATOR: A03\n");
	}
#endif
