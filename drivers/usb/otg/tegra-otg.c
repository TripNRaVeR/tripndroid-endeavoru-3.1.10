...
static void dump_otg_state(void)
{
	...
	USBH_INFO("%s(%d) USB_PHY_WAKEUP val = 0x%lx\n", __func__, __LINE__, val);
	if (val & (USB_VBUS_INT_EN | USB_ID_INT_EN)) {
		USBH_INFO("%s(%d) PHY_WAKEUP = 0x%lx\n", __func__, __LINE__, val);
	}
				...
				const char *buf, size_t count)
{
	...
	if (sscanf(buf, "%lx", &host) != 1 || host < 0 || host > 1)
		return -EINVAL;

	USBH_INFO("%s %lx ", __func__, host);
	if (host) {
